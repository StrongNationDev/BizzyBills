...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function populatePlans(networkId) {
    ...
    (DATA_PLANS[networkId] || []).forEach(plan => {
  ...
  form.addEventListener("submit", (e) => {
    ...
    const planText = offerSelect.selectedOptions[0].textContent;
    const amount = Number(planText.match(/₦([\d,.]+)/)[1].replace(/,/g, ""));
    ...
    document.getElementById("confirmOrder").onclick = () => {
      ...
      showModal(pinModal);
      startPINProcess(phone, planId, amount);
    };
    document.getElementById("cancelOrder").onclick = () => hideModal(confirmModal);
  });

  /** PIN PROCESS + FULL TRANSACTION LOGIC */
  function startPINProcess(phone, planId, amount) {
    let pinValue = "";
      ...
      key.onclick = async () => {
        ...
        if (key.classList.contains("confirm-pin-btn")) {
          ...
          if (!isSuccess) {
            ...
            showResult("❌ Failed", apiResponse.message || "Transaction failed");
            return;
          }

          /** STEP 4 — SUCCESS → SAVE HISTORY */
          const historyEntry = {
          ...
          showResult("✅ Success", "Your data has been delivered.");
          return;
        }
